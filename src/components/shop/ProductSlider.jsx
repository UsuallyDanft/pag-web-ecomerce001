...
const ProductSlider = ({ title, products }) => {
  ...
  const getCardWidth = () => {
    ...
    if (window.innerWidth < 600) return Math.floor((window.innerWidth - 120) / 1); // 1 tarjeta en móvil
    if (window.innerWidth < 900) return 220;
    return 250;
  };
  
  const cardWidth = getCardWidth();
  const gapWidth = typeof window !== 'undefined' && window.innerWidth < 600 ? 8 : 16;
